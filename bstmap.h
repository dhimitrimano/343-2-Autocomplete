...
public:
  ...
  void insertVector(const vector<value_type> &v);
...
private:
  ...
  void copyHelper(Node *n, const Node *nOriginal);

  // inserts a new node
  Node *&insert(value_type l , Node *n);

  // helper for rebalancing, it's a binary search method
  void rebalanceHelper(vector<value_type> &v, int low, int high);
  ...
  bool isEqual(Node *thisN, Node *otherN) const;

  // helper for getAll
  vector<value_type> &getHelper(Node *n, const key_type &k, vector<value_type> &v) const;

  // helper for height
  static void maxHeightHelper(int &largest, int current, const Node *n);

  // finds a node and makes ret into a reference of it if it exists
  void finderHelper(Node *n, const key_type &k, Node *&ret) const;
